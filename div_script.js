    ...
    for (const sym of symbols) {
        try {
            ...
            const res = await axios.get(
                `https://finnhub.io/api/v1/stock/metric?symbol=${sym}&metric=all&token=${API_KEY}`,
            );
            ...
            const divRes = await axios.get(
                `https://finnhub.io/api/v1/stock/dividend?symbol=${sym}&from=2023-01-01&to=2025-12-31&token=${API_KEY}`,
            );
