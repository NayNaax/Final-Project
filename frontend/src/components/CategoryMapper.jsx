...
export const CategoryMapper = ({ mapping, onMappingChange, allocations }) => {
    ...
    // Default to the first available allocation category so the form is usable immediately.
    const [category, setCategory] = useState(allocations[0]?.category || "");

    const handleAdd = () => {
        if (!symbol.trim() || !category) return;
        // Normalize symbols to uppercase so mappings stay consistent.
        onMappingChange({ ...mapping, [symbol.toUpperCase()]: category });
        setSymbol("");
    };

    const handleRemove = (sym) => {
        // Remove a single symbol mapping while preserving the rest.
    ...
    return (
        <div className={styles.container}>
            ...
            <p className={styles.subtitle}>
                Override default sectors by assigning specific stock symbols to your budget categories.
            </p>
            ...
            <div className={styles.formGroup}>
                <input
                    ...
                    value={symbol}
                    // Uppercase on input keeps UX aligned with saved symbol format.
                    onChange={(e) => setSymbol(e.target.value.toUpperCase())}
                    className={styles.input}
                />
                <select value={category} onChange={(e) => setCategory(e.target.value)} className={styles.select}>
                    {/* Only categories with values are selectable targets. */}
                    {allocations
                        .filter((a) => a.category)
                        .map((a) => (
            ...
            <div className={styles.list}>
                ...
                {/* Render each mapping as a removable badge row. */}
                {Object.entries(mapping || {}).map(([sym, cat]) => (
                    <div key={sym} className={styles.mappingItem}>
                        ...
                        <button type="button" onClick={() => handleRemove(sym)} className={styles.removeBtn}>
                            <X size={14} />
                        </button>
                    </div>
                ))}
