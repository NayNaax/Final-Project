...
export function SellReasonModal({ tradeId, symbol, onClose, onSuccess }) {
    ...
    const handleSubmit = async (e) => {
        ...
        setError('');
        try {
            const reasonString = notes.trim().length > 0 
        ...
        <div className={styles.modalOverlay}>
            <div className={styles.modalContainer}>
                ...
                <p className={styles.description}>
                    Congratulations on closing your position! Part of becoming a better investor is understanding <strong>why</strong> you make decisions. Log your reason below:
                </p>
                ...
                <form onSubmit={handleSubmit} className={styles.form}>
                    ...
                    <div className={styles.footer}>
                        <button type="button" onClick={onClose} className={styles.skipBtn}>Skip for now</button>
                        <button type="submit" disabled={submitting} className={styles.saveBtn}>
                            {submitting ? "Saving..." : <><Save size={18} /> Save Entry</>}
                        </button>
                    </div>
                </form>
            </div>
